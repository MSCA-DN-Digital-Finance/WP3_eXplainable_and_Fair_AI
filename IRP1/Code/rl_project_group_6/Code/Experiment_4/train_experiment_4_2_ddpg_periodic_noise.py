...
from generators import NoisyTrendPriceGenerator, CashPriceGenerator, NoisyPeriodicTrendPriceGenerator
...
generators = {
"UPWARD":  NoisyPeriodicTrendPriceGenerator(
        start=10.0,          # Base value (you can keep it the same or change for a different baseline)
        amplitude=5.0,       # Larger amplitude for larger waves
        frequency=0.2,       # Lower frequency for smoother, slower oscillations
        mu=0.0,              # Mean of the Gaussian noise (kept the same)
        sigma=0.7           # Increased noise for more erratic fluctuations
    ),  #
...
image_path = os.path.join(folder_path, "4_2_periodic_noise.png")
...
experiments_settings =  "4_2_ddpg_periodic_noise"
